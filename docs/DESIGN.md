...
**部门是原样透出的**：组织接口额外给了 `parentId` 就带上（本中心不加工、不校验父是否存在），
所以部门本身可以是棵树；面板的「服务树」页签用它把部门摆成层级，没有 `parentId` 就是扁平的。

**「服务树」页签**（面板）把上面两份数据拼成一棵可开合的树：部门（含**0 个服务的部门**，
因为要回答的是"这个组织里有什么"）→ 服务（叶子，展开就是它的对外 API 端点表）。
两条关键语义：**默认全部收起**；开合是**用户状态** —— 自动刷新（3s）重建 DOM 后原样保留，
且数据没变时连 DOM 都不重建（与「服务目录」的卡片展开同一套做法）。契约上没填部门的服务
收在最后一枝「未归属部门」，不因为"没写归属"就从树上消失。这是**纯前端视图**，不新增 API、不改数据模型。
