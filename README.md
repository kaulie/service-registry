...
部门字段是**原样透出**的：组织接口如果额外给了 `parentId`（说明部门自己就是一棵树，
比如集团 → 事业群 → 部门），本中心照样带出来、**不做层级加工也不校验父是否存在**，
由消费方决定怎么摆 —— 面板的「服务树」页签就是拿它把部门挂成层级；给不出来就是扁平的。
...
后端同端口托管，与 `/v1/*` 完全隔离）。两个页面共用一个 `shared.js`（令牌 / API 调用 /
toast / 部门目录 / 表单反馈），各自只放自己的逻辑：

| 页面 | 能力 |
|---|---|
| `/panel/`（控制面板） | 概览 / 服务目录 / 服务树 / API 检索 / 实例 / 变更与审计 / 命名空间 / 实时事件流（见下表） |
| `/panel/contract.html`（契约编辑页，独立页面） | **登记与编辑服务契约**都在这里：`contract.html` = 新登记，`contract.html?ns=<ns>&name=<服务>` = 编辑某个契约（URL 可收藏、可分享、可后退）；服务名在编辑时锁定（它是身份）；保存后页面给出「已保存」卡片（回面板 / 看内联 spec / 再登记一个），面板那边 3 秒自动刷新，切回去就是最新的 |
...
| 服务目录 | **「＋ 登记服务契约」**与卡片上的**「编辑契约」**都是**独立页面** `contract.html` 的链接（**新页签打开**，面板的滚动位置与展开状态不受影响）、**「＋ 加实例」**、服务卡片含 **API 端点表**（方法/路径/说明/标签/鉴权）+ 查看/下载内联 spec + 用真实实例生成调用示例；卡片可各自「展开/收起」，**自动刷新（3s）不会把已展开的卡片收起来**，多张卡片可以同时展开；契约可带 **gitRepoUrl（代码仓库）** 与**归属部门**，列表上直接显示可点的 `repo:` 与 `部门:` 标签，并可按部门过滤 |
| 服务树 | 独立页签：把**组织架构（部门）**与**服务契约**拼成一棵树 —— 部门（组织接口给了 `parentId` 就按层级嵌套；**0 个服务的部门也列出来**，好回答"这个组织里有什么"）→ 服务（展开就是它的对外 API 端点表）；**默认全部收起**，每行独立开合，「全部展开/收起」一次调整整棵树，另有关键字过滤（命中的节点自动展开）；**展开状态在 3s 自动刷新后保留**（数据没变时连 DOM 都不重建）。没填归属部门的服务收在最后一枝「未归属部门」，不会从树上消失 |
...
页签在地址栏里（`/panel/#services`）：刷新、分享、从契约编辑页的「← 返回面板」都能落回同一个页签。

**配色**：`web/styles.css` 顶部是一套颜色令牌 —— 中性灰阶（不带蓝调）+ **一个**强调色
（`--accent`，紫罗兰；深浅由 `--accent-soft` / `--accent-line` / `--accent` 三档决定），
状态色（`--ok` / `--warn` / `--bad`）只表达状态。深色是默认，右上角的 ☀/☾ 切到浅色
（`<html data-theme="light">` 换同一批令牌的取值），选择记在浏览器本地，两个页面共用。

契约表单的**反馈原则**：任何失败都不静默 —— 红色常驻提示 + toast + 出错字段标红并聚焦，
并注册了 `window.onerror` / `unhandledrejection` 兜底，脚本异常也会变成可见提示。
提交期间按钮进入"提交中…"禁用态，失败保留已填内容（成功才切到「已保存」卡片）。

> 两个页面都是纯前端，Go 测试覆盖不到它们的交互，所以有一份**受控 DOM 冒烟**：
> `make panel-smoke`（`node web/panel-smoke.mjs`，无 node 环境会跳过）—— 它按浏览器的加载顺序
> 把 `shared.js` + `app.js`（面板）/ `shared.js` + `contract.js`（契约编辑页）跑在受控 DOM 里，
> 模拟"填表 → 点提交"、"点开「展开」后自动刷新"、"打开服务树 → 默认收起 → 只展开被点的那一枝 →
> 自动刷新/数据变化后仍展开"、"带 `?ns=&name=` 打开编辑页 → 回填 → 提交"，
> 断言上面那些反馈行为与列表/树的状态。这个测试正是为
> "点了提交没反应、也不提示哪里错" 和 "展开的内容被自动刷新收起来" 这类问题加的。
>
> 想看**真的跑在服务上的那份面板**怎么做：`node web/panel-e2e.mjs http://127.0.0.1:4240`
> （`make panel-e2e`）—— 从服务上取回 `go:embed` 进去的 `/panel/shared.js` + `/panel/app.js`
> 与契约编辑页资源，在受控 DOM 里跑，fetch 直接打真实 API；默认只读（本服务是注册中心，
> 元信息就是数据）。
