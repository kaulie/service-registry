...
面板的**表单反馈原则**：任何失败都不静默 —— 红色常驻提示 + toast + 出错字段标红并聚焦，
并注册了 `window.onerror` / `unhandledrejection` 兜底，脚本异常也会变成可见提示。
提交期间按钮进入"提交中…"禁用态，成功才关闭表单、失败保留已填内容。

> 面板是纯前端，Go 测试覆盖不到它的交互，所以有一份**受控 DOM 冒烟**：
> `make panel-smoke`（`node web/panel-smoke.mjs`，无 node 环境会跳过）—— 它模拟"填表 → 点提交"、
> "点开「展开」后自动刷新"、"打开服务树 → 默认收起 → 只展开被点的那一枝 → 自动刷新/数据变化后仍展开"，
...
> （`make panel-e2e`）—— 从服务上取回 `go:embed` 进去的 `/panel/app.js`，在受控 DOM 里跑，
> fetch 直接打真实 API；默认只读（本服务是注册中心，元信息就是数据）。
